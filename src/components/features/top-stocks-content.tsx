...
export function TopStocksContent({
  stocks,
  totalCount,
}: {
  stocks: StockPreview[];
  ...
  const t = useTranslations("topStocks");
  const visible = stocks.slice(0, VISIBLE_COUNT);
    ...
    <div className="min-h-screen">
      ...
      <Container className="space-y-10 py-12 pb-20">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {visible.map((stock, i) => (
            <Card key={stock.symbol} variant="glass" className="flex flex-col gap-2">
              <div className="flex flex-wrap items-center gap-1.5">
                <Badge>
                  {getRegionFlag(stock.region.toLowerCase() as "us" | "europe" | "em" | "global")}
                </Badge>
                <Badge>
              ...
              <h3 className="text-lg font-semibold text-text-primary">
                {stock.symbol} &middot; {stock.name}
              </h3>
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            ...
            <Badge variant="accent">
              {t("moreStocks", { count: totalCount - VISIBLE_COUNT })}
            </Badge>
            <Link href="/login">
